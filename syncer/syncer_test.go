...
func (s *testSyncerSuite) mockCheckPointCreate(checkPointMock sqlmock.Sqlmock, isCreateTable bool) {
	...
	checkPointMock.ExpectExec(fmt.Sprintf("INSERT INTO `%s`.`%s_syncer_checkpoint`", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
	if isCreateTable {
		checkPointMock.ExpectExec(fmt.Sprintf("INSERT INTO `%s`.`%s_syncer_checkpoint_st`", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
	}
	// TODO because shardGroup DB is same as checkpoint DB, next time split them is better
	checkPointMock.ExpectExec(fmt.Sprintf("DELETE FROM `%s`.`%s_syncer_sharding_meta(228)?", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
	checkPointMock.ExpectCommit()
}

func (s *testSyncerSuite) mockCheckPointFlush(checkPointMock sqlmock.Sqlmock, isAlterTable bool) {
	checkPointMock.ExpectBegin()
	checkPointMock.ExpectExec(fmt.Sprintf("INSERT INTO `%s`.`%s_syncer_checkpoint`", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
	for i := 0; i < 2; i++ {
		checkPointMock.ExpectExec(fmt.Sprintf("INSERT INTO `%s`.`%s_syncer_checkpoint`", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	// TODO because shardGroup DB is same as checkpoint DB, next time split them is better
	checkPointMock.ExpectExec(fmt.Sprintf("DELETE FROM `%s`.`%s_syncer_sharding_meta(239)?", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
	checkPointMock.ExpectCommit()
}
...
func (s *testSyncerSuite) TestSharding(c *C) {
	...
	for i, _case := range testCases {
		...
		checkPointMock.ExpectExec(fmt.Sprintf("CREATE TABLE IF NOT EXISTS `%s`.`%s_syncer_checkpoint_st`", s.cfg.MetaSchema, s.cfg.Name)).WillReturnResult(sqlmock.NewResult(1, 1))
		checkPointMock.ExpectCommit()
		...
		s.mockCheckPointCreate(checkPointMock, false)
		s.mockCheckPointCreate(checkPointMock, true)
		s.mockCheckPointCreate(checkPointMock, true)
		...
		for i, expectSQL := range _case.expectSQLS {
			...
			if strings.HasPrefix(expectSQL.sql, "ALTER") {
				...
				mock.ExpectCommit()
				s.mockCheckPointFlush(checkPointMock, true)
			} else {
		...
		s.mockCheckPointFlush(checkPointMock, false)
